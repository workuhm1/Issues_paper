...
## prototype
# all_seed <- round(runif(50,30000,39999))   
all_seed <- round(runif(50,40000,49999))   
...
for(rep in all_seed){
  ...
  dgenerateInterval(N=A, P=B, rho_lat=C, lambda=D_lambda, seed=rep)
}
...
ls <- foreach(file = files_RData) %do% {
  ...
  dualModel(dataY=Ylat, dataXX = XX, file=file_out, seed=seed)
