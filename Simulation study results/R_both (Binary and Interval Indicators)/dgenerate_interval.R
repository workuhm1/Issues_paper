dgenerateInterval = function(N, P, rho_lat, lambda, seed){
    ...
    for(b in P){
      for(c in rho_lat){
        for(d in 1:ld){
            ...
            ## variance-covariance structure under MIMIC model
            psi11 = var(mu1)
            psi22 = var(mu2)            
            ...
            phi_mat = diag(b) - diag(c(lam1^2, lam2^2))
            ...
            ## ----------------------------------------------
            ## STEP-3: Finally, obtain the manifest variables
            ## ----------------------------------------------

            Ylat = theta%*%t(lambda_mat) + epsil

            colnames(Ylat) = paste("Y", 1:b, sep="")
            Ylat = as.data.frame(Ylat)

            XX = data0

#             save(a, b, c, d, Ylat, XX, theta, seed,
#                  lam1 = lam1, lam2 = lam2, psi_off = psi_off, psi11, psi22,
#                  file=paste(a,"-",b,"-",c,"-",d,"-",seed,".RData",sep=""))
            
            ## export
            fn <- paste(a,"-",b,"-",c,"-",d,"-",seed,".RData",sep="")
