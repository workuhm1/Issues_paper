dualModel <- function(dataY, dataXX, file, seed){
  ...
  mod1 <- mplusObject(
    ...
    SAVEDATA = "
      FILE is CFA_scores.txt;
      SAVE = fscores;",
    usevariables = Y_names,
    rdata = dataY)
  
  ## Specify the MIMIC model in Mplus
  varSpec = paste("CATEGORICAL =", paste(Y_names[1:b], collapse=" "), ";", sep=" ")
  ...
  ## save outputs, CFA
  res_CFA <- data.frame(seed=seed, a=a, b=b, c=c, d=d, e=e,
                        rho1=numeric(1), rho2=numeric(1), 
                        err1=logical(1), err2=logical(1),
                        flipped=character(1))
  
  ## save outputs, MIMIC
  res_MIMIC <- data.frame(seed=seed, a=a, b=b, c=c, d=d, e=e,
                          rho1=numeric(1), rho2=numeric(1), 
                          err1=logical(1), err2=logical(1),
                          flipped=character(1))  
  ...
  if(((res_CFA$err1 == FALSE) & (res_CFA$err2 == FALSE))&
    ...
    res_corrCFAMIMIC <- data.frame(seed=seed, a=a, b=b, c=c, d=d, e=e,
                                   rho1_CFAMIMIC=numeric(1), rho2_CFAMIMIC=numeric(1))  
    
    ## CFA
    f1_CFA = summCFA$savedata[ ,(b+1)]
    f2_CFA = summCFA$savedata[ ,(b+2)]
    
    ## MIMIC
    f1_MIMIC = summMIMIC$savedata[ ,(b+9)]
    f2_MIMIC = summMIMIC$savedata[ ,(b+10)]
